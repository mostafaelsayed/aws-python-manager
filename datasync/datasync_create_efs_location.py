...
def create_efs_location(efs_location_config):
    ...
    security_groups = attach_security_groups(efs_location_config['ec2_config'])
    for security_group in security_groups:
        security_groups_arns.append('arn:aws:ec2:{0}:{1}:security-group/{2}'.format(client.region, env['aws_account_id'], security_group))
    
    response = instance.create_location_efs(
        Ec2Config={
            'SubnetArn': 'arn:aws:ec2:{0}:{1}:subnet/{2}'.format(client.region, env['aws_account_id'], efs_location_config['ec2_config']['subnet']),
            'SecurityGroupArns': security_groups_arns,
        },
        EfsFilesystemArn='arn:aws:elasticfilesystem:{0}:{1}:file-system/{2}'.format(client.region, env['aws_account_id'], file_system_id),
    )
